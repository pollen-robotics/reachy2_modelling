...
def robot_from_urdf(filepath, link_names=None, gripper_name=None):
    links, name, urdf_string, urdf_filepath = rtb.Robot.URDF_read(filepath,
                                                                  tld='.')

    if gripper_name is not None and link_names is not None and gripper_name not in link_names:
...
links, name, urdf_string, urdf_filepath = rtb.Robot.URDF_read(urdf_path,
                                                              tld='.')
...
robot_l_arm = robot_from_urdf(urdf_filepath,
                              links_l_arm,
                              gripper_name='l_arm_tip')

print_debug('robot_r_arm')
robot_r_arm = robot_from_urdf(urdf_filepath,
                              links_r_arm,
                              gripper_name='r_arm_tip')

print_debug('robot_head')
robot_head = robot_from_urdf(urdf_filepath,
                             links_head,
                             gripper_name='head_tip')
